//! launchd socket activation. wtx starts when a pull arrives from a VM and exits
//! when idle, so no process needs to remain resident.
...
/// Receive a socket passed by launchd. Return None outside launchd so the caller
/// can fall back to a regular bind.
pub fn activated_listener(name: &str) -> Option<TcpListener> {
    ...
    unsafe {
        if launch_activate_socket(cname.as_ptr(), &raw mut fds, &raw mut cnt) != 0 || cnt == 0 {
            return None;
        }
        let fd = *fds;
        libc::free(fds.cast());
...
"#,
    ...
    // Bootstrap fails when an existing resident process holds the same port.
